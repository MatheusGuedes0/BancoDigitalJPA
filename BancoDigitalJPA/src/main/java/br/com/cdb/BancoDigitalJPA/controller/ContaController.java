...
public class ContaController {
    ...
    public ResponseEntity<String> addConta(@RequestBody Conta conta) {
        Conta contaAdicionada = contaService.salvarConta(conta.getNumero(), conta.getSaldo(),
                conta.getCliente(), conta.getTipoConta());
        if (contaAdicionada != null) {
            return new ResponseEntity<>("Conta " + conta.getNumero() + " adicionada com sucesso!",
                    HttpStatus.CREATED);
        } else {
